...
var boxSizeArray = [10]; // Array indicating how many items there is rooom for in the right column ULs
...
var navigatorVersion =
  navigator.appVersion.replace(/.*?MSIE (\d\.\d).*/g, "$1") / 1;
...
//  console.log("2");
  ...
  for (var no = 0; no < ulPositionArray.length; no++) {
    ...
    if (
      x + width > ul_leftPos &&
      x < ul_leftPos + ul_width &&
      y + height > ul_topPos &&
      y < ul_topPos + ul_height
    ) {
      var noExisting = ulPositionArray[no]["obj"].getElementsByTagName("LI")
        .length;
      if (
        indicateDestinationBox &&
      ...
      if (noExisting < boxSizeArray[no - 1] || no == 0) {
          ...
          if (!indicateDestionationByUseOfArrow) {
            ...
            if (y < tmpTop) {
              ...
              dragDropIndicator.style.top =
                tmpTop +
                tmpOffsetY -
                Math.round(dragDropIndicator.clientHeight / 2) +
                "px";
        ...
        if (!indicateDestionationByUseOfArrow) {
          ...
          if (subLi.length > 0 && dragDropIndicator.style.display == "none") {
            dragDropIndicator.style.top =
              getTopPos(subLi[subLi.length - 1]) +
...
/*  console.log("3",dragTimer);
  console.log("contentTBD",contentToBeDragged);
  console.log("DDI",dragDropIndicator);
  console.log("iDB",indicateDestinationBox);
  ...
  if (
    ...
    if (destinationObj) {
      if (destinationObj.tagName == "UL") {
        ...
        destinationObj.parentNode.insertBefore(
          contentToBeDragged,
          destinationObj
        );
      }
...
function saveDragDropNodes() {
  ...
  document.getElementById("saveContent").innerHTML =
    "<h1>Ready to save these nodes:</h1> " +
...
function initDragDropScript() {
  ...
  dragDropTopContainer = document.getElementById(
...
function checkForTea(){
  list_of_steps = document.getElementById("box1").childNodes;
  flag = 0;

  if(list_of_steps.length-1 != steps){
    return alert("Code is incomplete");
  }


  for(var i=1;i<list_of_steps.length;i++){
    temp = list_of_steps[i].id;
    if("node"+i.toString() != temp){
      flag = 1;
      break;
    }
  }
  result_block = document.getElementsByClassName("result-block");
  result_block[0].classList.add('visible');
  setTimeout(function(){
    document.getElementsByClassName("loading")[0].style.display = "none";

    result = document.getElementById("result-text");
    if(flag == 1){
      result.innerHTML = "Oops! Process is incorrect.<br>Try Again<div class='reload-btn'><button onclick='location.reload()'>Reload</button></div>";
    }
    else{
      result.innerHTML = "Yayy, you did it right!<br>See how easy coding is<div class='reload-btn'><button onclick=''>Join this club</button></div>";
    }
